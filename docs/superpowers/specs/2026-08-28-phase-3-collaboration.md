...
| **Shared memory** | All participating agents use the same governed memory worker backed by PostgreSQL/pgvector with MemPalace behind it. Component memory remains the repository-local `.agents/memory.md`; shared retrieval and publication use `system`, `domain`, and `org` scopes. Agent-originated system facts still require the D22 confirmation flow. |

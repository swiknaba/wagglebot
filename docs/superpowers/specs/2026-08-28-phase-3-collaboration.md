# Phase 3 — Cross-Machine Agent Collaboration
...
| **Task board** | Order tasks by `priority DESC, created_at ASC`. Give each claim a lease and a claim version (`fence`). Increase the version with each new claim. Require the current version for heartbeats and completion. Reject updates from expired or older claims. Return expired tasks to the queue within the attempt limit (D5). Use `idempotencyKey` to prevent duplicate external actions. Use the fields from [contract C4](2026-08-28-service-contracts.md#c4-task-envelope-and-delegated-job-vocabulary-phase-3). Require a `system` for each task. Permit an optional branch. |
| **Shared memory** | All participating agents point at the same memory-worker + Postgres (pgvector). `scopeIds` carry the `component` and the `system` of the workspace. Agent writes default to `component`, with confirmed promotion to `system` (D22). |

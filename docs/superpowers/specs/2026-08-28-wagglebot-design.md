...
| # | Decision |
|---|---|
| D1 | Application services and database-management tooling use **TypeScript (Bun)**. Migrations run as a separate one-shot deployment job. The hub uses `@modelcontextprotocol/sdk`. |
...
| D3 | The memory worker uses PostgreSQL with pgvector through a standard client. Require the [Phase 2 database tooling](2026-08-28-phase-2-shared-layer.md#shared-database-and-migrations), including the advisory-locked TypeScript/Bun migration job. Register the worker MCP interface with the hub. Do not add an MCP wrapper around PostgreSQL (P17). |

...
import { findGitRoot, findProjectRoot } from "./project-root";
...
test("throws outside any Git repository", () => {
  ...
  expect(() => findGitRoot(outside)).toThrow(/Git repository/);
});

test("findProjectRoot remains a compatibility export", () => {
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "wgl-repo-")));
  mkdirSync(join(repo, ".git"));

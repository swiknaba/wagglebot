...
const valid = {
  ...
  REGISTRY_AUTH_PUBLIC_KEY_FILE: "/run/secrets/registry-auth-public-key.pem",
...
test("loads strict registry configuration", () => {
  expect(loadRegistryConfig(valid)).toEqual({
    ...
    authPublicKeyFile: valid.REGISTRY_AUTH_PUBLIC_KEY_FILE,
...
test("loads documented registry settings from a process-like environment", () => {
  ...
  ).toMatchObject({ authPublicKeyFile: "/run/secrets/registry-auth-public-key.pem" });
});
